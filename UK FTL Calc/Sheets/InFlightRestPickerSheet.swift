...
struct InFlightRestPickerSheet: View {
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        // NEW: Check crew type to show appropriate options
                        if viewModel.crewType == "Cabin Crew" {
                            // CABIN CREW OPTIONS
                            cabinCrewRestOptions
                        } else {
                            // PILOT OPTIONS (existing logic - completely unchanged)
                            pilotRestOptions
                        }
                    }
    ...
    // NEW: Cabin Crew Rest Options
    private var cabinCrewRestOptions: some View {
        VStack(spacing: 20) {
            // Rest Facility Type Selection
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(spacing: 12) {
                    ...
                    Button(action: {
                        ...
                        // Don't dismiss sheet - let user continue to time selection
                    }) {
                        HStack {
                            VStack(alignment: .leading, spacing: 6) {
                                HStack {
            ...
            // In-Flight Rest Time Available (NEW for cabin crew)
            if viewModel.hasInFlightRest && viewModel.restFacilityType != .none {
                VStack(alignment: .leading, spacing: 16) {
                    HStack {
                        ...
                        Text("In-Flight Rest Time Available")
                            .font(.headline)
                            .fontWeight(.semibold)
                            .foregroundColor(.primary)
                    }
                    
                    let availableOptions = CabinCrewInFlightRestTable.getAvailableRestTimeOptions(for: viewModel.restFacilityType)
                    
                    VStack(spacing: 12) {
                        ForEach(availableOptions, id: \.self) { restTime in
                            Button(action: {
                                viewModel.inFlightRestTimeAvailable = restTime
                            }) {
                                HStack {
                                    VStack(alignment: .leading, spacing: 4) {
                                        Text(CabinCrewInFlightRestTable.formatRestTime(restTime))
                                            .font(.headline)
                                            .fontWeight(.semibold)
                                            .foregroundColor(.primary)
                                        
                                        Text("Available rest time")
                                            .font(.caption)
                                            .foregroundColor(.secondary)
                                    }
                                    ...
                                    if abs(viewModel.inFlightRestTimeAvailable - restTime) < 0.01 {
                                        Image(systemName: "checkmark.circle.fill")
                                            .foregroundColor(.orange)
                                    }
                                }
                                ...
                                .background(
                                    RoundedRectangle(cornerRadius: 12)
                                        .fill(abs(viewModel.inFlightRestTimeAvailable - restTime) < 0.01 ? Color.orange.opacity(0.1) : Color(.systemGray6))
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 12)
                                                .stroke(abs(viewModel.inFlightRestTimeAvailable - restTime) < 0.01 ? Color.orange : Color(.systemGray4), lineWidth: 2)
                                        )
                                )
                            }
                            .buttonStyle(PlainButtonStyle())
            ...
            // Maximum FDP Display for Cabin Crew
            if viewModel.hasInFlightRest && viewModel.restFacilityType != .none {
                VStack(alignment: .leading, spacing: 16) {
                    HStack {
                        ...
                        Text("Maximum Allowed FDP")
                            .font(.headline)
                            .fontWeight(.semibold)
                            .foregroundColor(.green)
                    }
                    
                    let maxFDP = CabinCrewInFlightRestTable.lookupMaxFDP(
                        restFacilityType: viewModel.restFacilityType,
                        restTimeAvailable: viewModel.inFlightRestTimeAvailable
                    )
                    ...
                    VStack(spacing: 12) {
                        ...
                        HStack {
                            Text("Rest Time Available:")
                                .font(.subheadline)
                                .foregroundColor(.secondary)
                            Spacer()
                            Text(CabinCrewInFlightRestTable.formatRestTime(viewModel.inFlightRestTimeAvailable))
                                .font(.subheadline)
                                .fontWeight(.medium)
                                .foregroundColor(.primary)
                        }
                        ...
                        HStack {
                            Text("Maximum FDP:")
                                ...
                                .foregroundColor(.green)
                            Spacer()
                            Text(CabinCrewInFlightRestTable.getMaxFDPDisplayString(
                                restFacilityType: viewModel.restFacilityType,
                                restTimeAvailable: viewModel.inFlightRestTimeAvailable
                            ))
                                .font(.title3)
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack {
                            Image(systemName: "info.circle.fill")
                                .foregroundColor(.green)
                                .font(.caption)
                            Text("Table 5 Reference")
                                .font(.caption)
                                .fontWeight(.medium)
                                .foregroundColor(.green)
                        }
                        
                        Text("Based on British Airways OM A Table 5: Minimum Required In-flight Rest for Cabin Crew Members")
                            .font(.caption)
                            .foregroundColor(.secondary)
            ...
            // Completion Button for Cabin Crew
            if viewModel.hasInFlightRest && viewModel.restFacilityType != .none {
                VStack(spacing: 16) {
                    Button(action: {
                        isPresented = false
                    ...
                    Text("Review your selections above before completing")
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
                }
    ...
    // EXISTING PILOT OPTIONS - COMPLETELY UNCHANGED
    private var pilotRestOptions: some View {
        VStack(spacing: 20) {
